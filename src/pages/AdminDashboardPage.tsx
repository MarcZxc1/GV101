...
export function AdminDashboardPage() {
  ...
  const [nextStatus, setNextStatus] =
    useState<ProviderVerificationStatus>("approved");
    ...
    <div className="grid gap-6">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-slate-600">
              Lightweight UI to demonstrate SRS admin concepts (verification +
      ...
      <section className="section">
        <div className="text-lg font-semibold text-slate-900">
          Provider verification (simulated)
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <div className="card bg-slate-50 p-4 text-sm text-slate-700 ring-1 ring-slate-200">
            ...
            <div className="mt-1">
              This demo doesn’t persist admin decisions yet (no backend). If you
              want, I’ll wire admin actions into the in-memory store next.
            </div>
          </div>
        </div>
        {provider ? (
          <div className="card mt-4">
            ...
            <div className="mt-1 text-sm text-slate-600">
              Status: <b>{provider.verificationStatus}</b> · Category:{" "}
              {provider.category} · Barangay: {provider.barangay}
            </div>
            <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-2">
                ...
                <select
                  value={nextStatus}
                  onChange={(e) =>
                    setNextStatus(e.target.value as ProviderVerificationStatus)
                  }
              ...
              <button
                ...
                onClick={() =>
                  actions.setProviderVerification(provider.id, nextStatus)
                }
              >
                Apply
              </button>
      ...
      <section className="section">
        <div className="text-lg font-semibold text-slate-900">
          Flagged reviews (moderation demo)
        </div>
        <div className="mt-2 text-sm text-slate-600">
          SRS rule: after <b>3 flags</b>, review should be hidden pending admin
          decision.
        </div>
              ...
              <div key={r.id} className="card">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="text-xs text-slate-500">
                    Flags: <b>{r.flags.count}</b> · Hidden:{" "}
                    <b>{String(r.flags.hidden)}</b>
                  </div>
                </div>
