    ...
    <div className="grid gap-6">
      <section className="rounded-3xl border border-slate-200 bg-white p-7">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-2xl font-semibold text-slate-900">Customer Portal</h2>
            <p className="mt-1 text-slate-600">
              Track active bookings and history. Reviews require 50+ characters to stay useful.
            </p>
          </div>
    ...
    <div className="rounded-2xl border border-slate-200 p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          {b.status === 'requested' ? (
            <div className="mt-2 text-xs text-slate-500">
              Provider response due in: <span className="font-semibold">{remainingMin} min</span>{' '}
              (auto-decline after 30 minutes)
            </div>
          ) : null}
          {b.payment ? (
            <div className="mt-2 text-xs text-slate-500">
              Payment: <span className="font-semibold">{b.payment.status}</span> · Method:{' '}
