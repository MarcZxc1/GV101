...
type Action =
  ...
  | { type: 'provider.setVerification'; providerId: string; status: ProviderVerificationStatus }
  ...
  switch (action.type) {
                ...
                payment:
                  action.status === 'accepted'
                    ...
                    : action.status === 'completed'
                      ? { ...b.payment, status: 'released', receiptId: b.payment.receiptId ?? `rcpt_${b.id}` }
                      : b.payment,
              }
            : b,
        ),
    ...
    case 'booking.cancel':
      return {
        ...
        bookings: state.bookings.map((b) =>
          b.id === action.bookingId
            ? { ...b, status: 'cancelled', cancelledAt: action.at, cancelReason: action.reason }
            : b,
        ),
      }
    case 'booking.reschedule':
      return {
        ...state,
        bookings: state.bookings.map((b) =>
          b.id === action.bookingId ? { ...b, scheduledFor: action.to, rescheduledFrom: action.from } : b,
        ),
      }
    ...
    case 'review.respond': {
      return {
        ...
        reviews: state.reviews.map((r) => {
          ...
          return { ...r, providerResponse: { text: action.text, createdAt: action.at } }
        }),
      }
    }
    case 'provider.setVerification': {
      return {
        ...
        providers: state.providers.map((p) =>
          p.id === action.providerId ? { ...p, verificationStatus: action.status } : p,
        ),
      }
    }
    case 'message.send':
...
type Store = {
  ...
  actions: {
    ...
    cancelBooking: (bookingId: string, reason: string) => { ok: boolean; message?: string }
    rescheduleBooking: (bookingId: string, newIso: string) => { ok: boolean; message?: string }
    submitReview: (args: {
      bookingId: string
      providerId: string
    ...
    sendMessage: (args: { bookingId: string; sender: Message['sender']; text: string }) => Message
    flagReview: (reviewId: string) => void
    respondToReview: (reviewId: string, text: string) => { ok: boolean; message?: string }
    setProviderVerification: (providerId: string, status: ProviderVerificationStatus) => void
  }
}
    ...
    return {
      ...
      actions: {
        ...
        cancelBooking(bookingId, reason) {
          ...
          if (hours < 2) {
            return { ok: false, message: 'Cancellations are blocked within 2 hours of start (SRS rule).' }
          }
          dispatch({ type: 'booking.cancel', bookingId, reason, at: now.toISOString() })
          dispatch({
            type: 'notify.push',
            notification: {
              id: uid('ntf'),
        ...
        rescheduleBooking(bookingId, newIso) {
          ...
          const hours = (start.getTime() - now.getTime()) / 36e5
          if (hours < 4) {
            return { ok: false, message: 'Rescheduling is blocked within 4 hours of start (SRS rule).' }
          }
          dispatch({ type: 'booking.reschedule', bookingId, from: b.scheduledFor, to: newIso })
          dispatch({
            type: 'notify.push',
            notification: {
              id: uid('ntf'),
        ...
        submitReview({ bookingId, providerId, ratings, remarks, photos }) {
          ...
          dispatch({ type: 'booking.attachReview', bookingId, reviewId: review.id })
          return review
        },
        ...
        respondToReview(reviewId, text) {
          const t = text.trim()
          if (t.length === 0) return { ok: false, message: 'Response is empty.' }
          if (t.length > 300) return { ok: false, message: 'Max 300 characters (SRS).' }
...
function redactContacts(s: string) {
  ...
  const redactedUrls = redactedPhone.replace(/\bhttps?:\/\/\S+\b/gi, '[redacted link]')
  return redactedUrls
}
